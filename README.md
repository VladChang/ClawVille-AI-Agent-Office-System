...
### 已知問題
- 完整的 backend `npm run test` 在較大的 API/auth/audit 測試範圍仍有 hanging 問題。CI 目前刻意固定使用較穩定的 scoped backend test set，也就是 `backend npm run test:ci`，直到剩餘 hanging case 被修掉為止。
